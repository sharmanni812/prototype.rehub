...
class RehubApp:
    ...
    def show_login(self):
        ...
        login_field = ft.TextField(label="Email, ID или имя пользователя", width=400, autofocus=True)
    ...
    def show_dashboard(self):
        ...
        logout_button = ft.IconButton(icon=ft.Icons.LOGOUT, icon_size=24, on_click=lambda _: self.show_login(), tooltip="Выйти из аккаунта")
        top_bar = ft.Row([ft.Container(expand=True), logout_button, self.theme_button])
        nav_menu = ft.Column([
        ...
        search_functions = ft.TextField(hint_text="Поиск...", width=300, on_change=self.search_functions)
        ...
        top_row = ft.Row([right_panel_top, stats_container], alignment=ft.MainAxisAlignment.SPACE_BETWEEN, spacing=20)
        self.suggestions_column = ft.Column(spacing=5)
        self.fixed_height_container = ft.Container(content=self.suggestions_column, height=150, visible=True)
        right_panel_full = ft.Column([top_row, self.fixed_height_container], spacing=10)
        content = ft.Row([
    ...
    def show_settings(self):
        ...
        content = ft.Column([
            ...
            ft.Column([
                ...
                ft.FilledButton("Удалить аккаунт", on_click=delete_account, width=250, bgcolor=ft.Colors.RED_700, color=ft.Colors.WHITE),
            ], horizontal_alignment=ft.CrossAxisAlignment.CENTER, spacing=15),
            ft.Container(expand=True),
        ], expand=True)
        self.clear_and_add(content)
    ...
    def show_find_projects(self):
        self.clear()
        self.page.horizontal_alignment = ft.CrossAxisAlignment.CENTER
        search_field = ft.TextField(label="Поиск проектов", hint_text="Введите название", width=400)
        category_filter = ft.Dropdown(
            label="Категория",
            options=[ft.dropdown.Option("Все"), ft.dropdown.Option("IT"), ft.dropdown.Option("Media"), ft.dropdown.Option("Fashion")],
            value="Все", width=200
        )
        ...
        self.page.add(
            ft.Column([
                ft.Row([ft.IconButton(ft.Icons.HOME, on_click=lambda _: self.show_dashboard()), ft.Container(expand=True), self.theme_button]),
                ft.Column([
                    ft.Text("Поиск проектов", size=32),
    ...
    def show_project_detail_page(self, project):
        already_applied = self.session.query(Application).filter(
            Application.user_id == self.current_user.id, Application.project_id == project.id, Application.status == "Ожидание"
        ).first() is not None
    ...
    def show_my_projects_list(self):
        ...
        leader_expansion = ft.ExpansionTile(
            ...
            controls=[ft.ListTile(title=ft.Text(p.title), subtitle=ft.Text(f"Статус: {p.needed_roles}"), on_click=lambda e, proj=p: show_project_details(proj)) for p in as_leader] if as_leader else [ft.Text("Нет проектов")]
        )
        member_expansion = ft.ExpansionTile(
            title=ft.Text(f"Проекты, где я участник ({len(as_member)})"),
            controls=[ft.ListTile(title=ft.Text(p.title), subtitle=ft.Text(f"Лидер: {p.user.name}"), on_click=lambda e, proj=p: show_project_details(proj)) for p in as_member] if as_member else [ft.Text("Нет проектов")]
        )
        self._create_theme_button()
        content = ft.Column([
            ft.Row([ft.IconButton(ft.Icons.HOME, on_click=lambda _: self.show_dashboard()), ft.Container(expand=True), self.theme_button]),
    ...
    def show_applications_list(self):
        ...
        status_filter = ft.Dropdown(
            label="Статус заявок",
            options=[ft.dropdown.Option("Все"), ft.dropdown.Option("Ожидание"), ft.dropdown.Option("Принят"), ft.dropdown.Option("Отклонен")],
            value="Все", width=200,
        )
        ...
        def load_applications():
            ...
            incoming_apps = self.session.query(Application).join(Project).filter(Project.leader_id == self.current_user.id, Application.status == "Ожидание").all()
            ...
            self.page.add(
                ft.Column([
                    ft.Row([ft.IconButton(ft.Icons.HOME, on_click=lambda _: self.show_dashboard()), ft.Container(expand=True), self.theme_button]),
                    ft.Column([
                        ft.Text("Заявки", size=32),
                        status_filter,
                        ft.ExpansionTile(title=ft.Text(f"Отправленные мной заявки ({len(my_apps)})"), controls=my_apps_controls if my_apps else [ft.Text("Нет заявок")]),
                        ft.ExpansionTile(title=ft.Text(f"Входящие заявки ({len(incoming_apps)})"), controls=incoming_controls if incoming_apps else [ft.Text("Нет заявок")]),
                    ], horizontal_alignment=ft.CrossAxisAlignment.CENTER, spacing=15),
                ], expand=True)
            )
            self.page.update()
        ...
        self.page.add(
            ft.Column([
                ft.Row([ft.IconButton(ft.Icons.HOME, on_click=lambda _: self.show_dashboard()), ft.Container(expand=True), self.theme_button]),
                ft.Column([ft.Text("Заявки", size=32), status_filter], horizontal_alignment=ft.CrossAxisAlignment.CENTER, spacing=15),
            ], expand=True)
        )
        self.page.update()
        load_applications()
    ...
    def show_user_profile_page(self, user):
        ...
        projects_column = ft.Column(projects_controls, spacing=10) if projects_controls else ft.Text("Нет проектов")

        stats_container = ft.Container(
            content=ft.Column([
                ft.Text("Статистика пользователя", weight=ft.FontWeight.BOLD),
                ft.Text(f"Веду проектов: {stats['owned']}"),
                ft.Text(f"Участвует: {stats['joined']}"),
                ft.Text(f"Ожидают ответа: {stats['pending']}"),
            ], spacing=5),
            padding=15,
            bgcolor=ft.Colors.BLUE_GREY_900 if self.theme_mode == ft.ThemeMode.DARK else ft.Colors.GREY_200,
            border_radius=10,
        ...
        content = ft.Column([
            ...
            ft.Container(expand=True),
            ft.Column([
                ft.Text(user.name, size=32),
                ft.Text(f"ID: {user.id}", size=14, color=ft.Colors.GREY_400),
                ft.Text(f"Навыки: {user.skills or 'Не указаны'}"),
                ft.Text(f"О себе: {user.bio or 'Не указано'}"),
                ft.Divider(height=20),
                stats_container,
                ft.Divider(height=20),
                ft.Text("Проекты пользователя:", weight=ft.FontWeight.BOLD),
                ft.Text(f"Всего проектов: {len(all_projects)}"),
                ft.Divider(height=10),
                projects_column,
            ], horizontal_alignment=ft.CrossAxisAlignment.CENTER, spacing=15),
            ft.Container(expand=True),
    ...
    def show_create_project(self):
        ...
        category_field = ft.Dropdown(label="Категория", options=[ft.dropdown.Option(c) for c in ["IT", "Media", "Fashion"]], width=400, value="IT")
        ...
        def create(e):
            if not title_field.value:
                ...
                self.page.update()
                return
            services.create_project(self.session, title_field.value, desc_field.value or "", self.current_user.id, category_field.value, roles_field.value or "")
        ...
        content = ft.Column([
            ...
            ft.Container(expand=True),
            ft.Column([
                ft.Text("Создание проекта", size=32),
                title_field,
                desc_field,
                category_field,
                roles_field,
